...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  let sum = sumOfThreeNumbers(a,b,c);
  let product = multiplyOfThreeNumbers(a,b,c);
...
function multiplyOfThreeNumbers(a,b,c) {
  return multiply(multiply(a,b)[0], c)[0];
}
...
function sumArray(sumArr) { //eslint-disable-line
  ...
  for ( let i = 0; i < sumArr.length; i++) {
    sum = sumOfTwoNumbers(sum, sumArr[i]);
  }
...
function multiplyArray(testArray) { //eslint-disable-line
  ...
  for (let i = 0; i < testArray.length; i++) {
    product = testMultiplyArray(product, testArray[i])[0];
  }
  let productString = 'The number ' + testArray.join(',') + ' have a product of ' + product + '.';
  return [product, productString];
}
function testMultiplyArray(a,b) {
  return [a * b];
}
